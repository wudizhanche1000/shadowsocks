...
public class TableCipherTest {
    ...
    public TableCipherTest() {
    }

    @Test
    public void testTableResult() throws Exception {
        Cipher cipher = CipherFactory.getCipher("table");
        cipher.init(Cipher.DECRYPT, "foobar!".getBytes(), null);
        Field decryptField = cipher.getClass().getDeclaredField("decryptTable");
        decryptField.setAccessible(true);
        Field encryptField = cipher.getClass().getDeclaredField("encryptTable");
        encryptField.setAccessible(true);

        byte[] decryptTable = (byte[]) decryptField.get(cipher);
        byte[] encryptTable = (byte[]) encryptField.get(cipher);
        for (int i = 0; i < encryptTable.length; i++) {
            assert (byte) target1[0][i] == encryptTable[i];
            assert (byte) target1[1][i] == decryptTable[i];
        }
    }

    @Test
    public void testEncryption() throws Exception {
        Cipher decryptCipher = CipherFactory.getCipher("table");
        Cipher encryptCipher = CipherFactory.getCipher("table");
        decryptCipher.init(Cipher.DECRYPT, "barfoo!".getBytes(), null);
        encryptCipher.init(Cipher.ENCRYPT, "barfoo!".getBytes(), null);
        System.out.println(Hex.encodeHexString(encryptCipher.update("123456".getBytes())));
        CipherTestUtils.runCipher(encryptCipher, decryptCipher);
    }
}
